...
impl CommentDb {
    ...
    pub fn edit_comment(&self, id: CommentId, comment: &str) -> Result<(), DbError> {
        let content = sanitize_string(comment);
    ...
    pub fn get_post_with_ident(&self, ident: &str) -> Result<Post, DbError> {
        ...
        Ok(row.query_one((ident,), Post::from_sqlite_row)?)
    }
    ...
    pub fn add_shout(&self, user_id: UserId, content: &str) -> Result<ShoutId, DbError> {
        let content = sanitize_string(content);
        let mut statement = self.conn.prepare(INSERT_SHOUT_RETURNING_ID)?;
        let shout_id = statement.query_one((user_id, content), ShoutId::from_sqlite_row)?;
        Ok(shout_id)
    }
...
fn sanitize_name(input: &str) -> StackString {
    let mut content = StackString::new();
    pulldown_cmark_escape::escape_html_body_text(&mut content, input.trim()).unwrap();
    content
}
